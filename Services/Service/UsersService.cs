...
namespace Services
{
    public class UsersService : IUsersService
    {
                ...
                if (user != null)
                {
                    if (user.UserId != "0") // Update
                    {
                        ...
                        if (userInDB != null)
                        {
                            ...
                            {
                                bool previousStatus = userInDB.Active;
                                // edit
                                userInDB.FirstName = user.FirstName;
                                userInDB.LastName = user.LastName;
                                userInDB.Active = user.Active;
                                ...
                                var Decryptpwsd = userInDB.PasswordHash.VerifyWithBCrypt("asdfgh@123");
                                if (previousStatus != user.Active && user.Active && Decryptpwsd)
                                {
                    ...
                    else // Add
                    {
                        ...
                        else
                        {
                            ...
                            user.PasswordHash = userPassword.WithBCrypt();
        ...
        public async Task<ResponseMessage> UpdateAccountStatus(bool status, string id)
        {
            ...
            if (userInDB != null)
            {
                var previousStatus = userInDB.Active;
                userInDB.Active = status;
                _unitOfWork.UserRepository.Update(userInDB);
                _unitOfWork.Complete();
                var Decryptpwsd = userInDB.PasswordHash.VerifyWithBCrypt("asdfgh@123");
                if (previousStatus != status && status && Decryptpwsd)
                {
                    await _emailService.UserRegisterEmail(userInDB);
                }
                responseMessage.Success = true;
        ...
        public List<RoleVM> GetAllRoles(bool activeRoles = false)
        {
            ...
            if (activeRoles)
            {
                roles = _roleManager.Roles.Where(c => c.Active).Select(c => new RoleVM { Id = c.Id, Name = c.Name, Active = c.Active }).ToList();
            }
            else
            {
                roles = _roleManager.Roles.Select(c => new RoleVM { Id = c.Id, Name = c.Name, Active = c.Active }).ToList();
            }
            return roles;
        }
